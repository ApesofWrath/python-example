...
class SwerveModule:
    def __init__(
        ...
        cc_cfg.magnet_sensor.absolute_sensor_range = (
            configs.MagnetSensorConfigs().absolute_sensor_discontinuity_point
        )
        cc_cfg.magnet_sensor.sensor_direction = (
            signals.SensorDirectionValue.COUNTER_CLOCKWISE_POSITIVE
        )
        cc_cfg.magnet_sensor.magnet_offset = offset.m_as(
            "turn"
        )  # do this in Phoenix Tuner X instead
        ...
        turn_fx_cfg.feedback.feedback_sensor_source = (
    ...
    def update(self) -> None:
        self.position = wpimath.kinematics.SwerveModulePosition(
            (
                self.driveMotor.get_position().refresh().value
                * unit.turn
                * constants.kWheelCircumference
            ).m_as("meter"),
            wpimath.geometry.Rotation2d(
                (self.turningMotor.get_position().refresh().value * unit.turn).m_as(
                    "radian"
                )
            ),
        )
        self.state = wpimath.kinematics.SwerveModuleState(
            (
                self.driveMotor.get_velocity().refresh().value
                * unit.turn
                / unit.second
                * constants.kWheelCircumference
            ).m_as("meter / second"),
    ...
    def setDesiredState(
        ...
        target_speed = (
            state.speed * unit.meter / unit.second / constants.kWheelCircumference
        ).m_as("turn / second")
