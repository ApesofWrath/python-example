# project imports
import constants
from subsystems import Drivetrain, Turntable, Spinner
...
class RobotContainer:
    ...
    def __init__(self, isReal: bool = True) -> None:
        ...
        self.robotDrive = Drivetrain(isReal=isReal)
        ...
        self.driverController = commands2.button.CommandXboxController(
            constants.Global.kDriverControllerPort
        )
        ...
        self.robotDrive.setDefaultCommand(
            ...
            commands2.RunCommand(
                lambda: self.robotDrive.drive(
                    xSpeed=-wpimath.applyDeadband(
                        self.driverController.getRawAxis(1), 0.1
                    )
                    * constants.Drive.kMaxSpeed,
                    ySpeed=-wpimath.applyDeadband(
                        self.driverController.getRawAxis(0), 0.1
                    )
                    * constants.Drive.kMaxSpeed,
                    rot=-wpimath.applyDeadband(self.driverController.getRawAxis(4), 0.1)
    ...
    def configureButtonBindings(self) -> None:
        ...
        self.driverController.a().onTrue(self.turntable.freespin(2)).onFalse(
            self.turntable.freespin(0)
        )
        self.driverController.b().onTrue(self.turntable.freespin(-2)).onFalse(
            self.turntable.freespin(0)
        )
